...
public class ToggleActiveCommandHandler : IRequestHandler<ToggleActiveCommand>
{
  ...
  {
    Persistence.Entities.Category? category = await _context.Categories.FindAsync(request.Id, cancellationToken) ?? throw new CategoryNotFoundException(request.Id);
