...
public class GetCategoryDaysQueryHandler : IRequestHandler<GetCategoryDaysQuery, List<CategoryDayModel>>
{
  ...
  {
    List<CategoryModel> categories = await GetAllCategories(cancellationToken);
    ...
    {
      string categoryName = categories.FirstOrDefault(c => c.Id == categoryId)?.FullName ?? "Unknown";
  ...
  {
    List<Persistence.Entities.Category> categories = await _context.Categories.Include(c => c.Parent).ToListAsync(cancellationToken);
