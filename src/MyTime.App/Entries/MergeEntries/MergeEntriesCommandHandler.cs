...
public class MergeEntriesCommandHandler : IRequestHandler<MergeEntriesCommand, EntryModel>
{
	...
	{
		var primaryEntry = await _context.Entries.FindAsync(request.Primary);
		var secondaryEntry = await _context.Entries.FindAsync(request.Secondary);

		if (primaryEntry is null) throw new ValidationException();
		if (primaryEntry.IsDeleted) throw new ValidationException();
		if (secondaryEntry is null) throw new ValidationException();
		if (secondaryEntry.IsDeleted) throw new ValidationException();
		if (primaryEntry.OnDate != secondaryEntry.OnDate) throw new ValidationException();

		// primaryEntry.OnDate is not modified
		// primaryEntry.Description is not modified
		// duration should be the sum of the two values
		primaryEntry.Duration = primaryEntry.Duration + secondaryEntry.Duration;
