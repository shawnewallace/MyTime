...
public class ReportEndpoints : EndpointBase, ICarterModule
{
  ...
  {
    RouteGroupBuilder group = app.MapGroup("report").WithName("report-endpoints");
  ...
  public static async Task<IResult> CategorySummaryByWeek(string from, string to, IMediator mediator)
  {
    ...
    List<CategorySummaryModel> result = await mediator.Send(request: new GetCategoryReportByWeekQuery(start, end));
  ...
  public static async Task<IResult> CategorySummaryByDay(string from, string to, IMediator mediator)
  {
    ...
    List<DaySummaryModel> result = await mediator.Send(request: new GetDaySummaryQuery(start, end));
